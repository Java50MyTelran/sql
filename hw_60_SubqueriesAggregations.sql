...
-- SELECT interval_number * 10 as min, 10 * (interval_number + 1) as max, count(*) as amount FROM (SELECT floor(date_part('year',age(birth_date)) / 10) as interval_number FROM car_owners)
